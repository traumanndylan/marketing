...
export class SessionService implements OnModuleDestroy, OnModuleInit {
  ...
  /**
   * On backend startup, automatically restart previously active sessions
   */
  async onModuleInit(): Promise<void> {
    ...
    if (sessionsToRestart.length > 0) {
      this.logger.log(`Auto-starting ${sessionsToRestart.length} session(s) on startup...`, {
        action: 'startup_autostart',
        count: sessionsToRestart.length,
      });
      ...
      for (const session of sessionsToRestart) {
        try {
          ...
          this.logger.error(`Failed to auto-start session ${session.name}`, String(error), {
            sessionId: session.id,
          });
          // Update status to disconnected if it fails
          await this.updateStatus(session.id, SessionStatus.DISCONNECTED);
        }
      }
    }
  }
