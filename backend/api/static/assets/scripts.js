function showToast(message, type = 'info') {
        ...
        async function fetchOverview() {
            ...
            <div class="stat" style="grid-column: span 4;">
                ...
                if (d.country_stats) {
                    const countries = Object.entries(d.country_stats).map(([c, count]) => `${c}: ${count}`);
                    document.getElementById('countryStats').innerText = countries.join(' | ');
                }
            } catch (e) { }
        }
        ...
        async function fetchScraperEta() {
            try {
                ...
                if (d.eta) {
                    document.getElementById('scraperEta').innerText = d.eta;
                } else {
                    document.getElementById('scraperEta').innerText = "—";
                }
            } catch (e) {}
        }
